...
@OnlyIn(Dist.CLIENT)
public class FlaxyRender<T extends EntityAbstractSummonedSword> extends EntityRenderer<T> {
    private static final ResourceLocation TEXTURE = Legendblade.prefix("model/util/ss.png");
    private static final ResourceLocation MODEL = Legendblade.prefix("model/util/sstar.obj");
    ...
    public FlaxyRender(EntityRendererProvider.Context context) {
        super(context);
    }
    ...
    public void render(T entity, float entityYaw, float partialTicks, PoseStack matrixStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        try (MSAutoCloser ignored = MSAutoCloser.pushMatrix(matrixStack)) {
            ...
            BladeRenderState.renderOverridedLuminous(ItemStack.EMPTY, model, "ss", TEXTURE, matrixStack, bufferIn,
                    packedLightIn);
        }
    }
}