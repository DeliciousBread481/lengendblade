...
public class SevenSwordSkill {
    ...
    public static void appluA(PlayerInteractEvent.LeftClickEmpty event){
        ...
        /*@SubscribeEvent
        public static void  keyEvent(InputCommandEvent event){
            ItemStack mainHandItem = event.getEntity().getMainHandItem();
            CompoundTag tag = mainHandItem.getTag();
            if (tag!=null) {
                if (tag.contains("UnLock")){
                    EnumSet<InputCommand> old = event.getOld();
                    if (old.contains(InputCommand.L_DOWN) || old.contains(InputCommand.L_CLICK)){
                        if (old.contains(InputCommand.SNEAK)) {
                            DemonBladeMessageA.apply(event.getEntity());
                        }else {
                            DemonBladeMessage.apply(event.getEntity());
                        }
    ...
    public static void slash(SlashBladeEvent.DoSlashEvent event){
       /* CompoundTag tag = event.getBlade().getTag();
        ...
        LivingEntity entity = event.getUser();
            DemonBladeMessageA.apply(entity);
              ...
              for (Entity entity2 : _entfound){
                ...
                if (entity2 instanceof SevenSkillField sevenSkillField){
                    ...
                    for (int i = 0; i < random.nextInt(3)+1; i++) {
                        ...
                        drive.getDimensions(drive.getPose()).scale(36, 36);
