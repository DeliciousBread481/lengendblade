...
public class SommonSwordSommon {
    ...
    public static void doSlash(LivingEntity playerIn, int colorCode, boolean critical, double damage, float speed,int deplays,int count)
    {
        ...
        playerIn.getMainHandItem().getCapability(ItemSlashBlade.BLADESTATE).ifPresent((state) -> {
            ...
            int rank = playerIn.getCapability(CapabilityConcentrationRank.RANK_POINT).map(r -> r.getRank(worldIn.getGameTime()).level).orElse(0);
              int extra=  playerIn.getMainHandItem().getItem().getEnchantmentLevel(playerIn.getMainHandItem(), Enchantments.POWER_ARROWS);
              int extra2;
              if (playerIn.hasEffect(MobEffects.DAMAGE_BOOST)){
                  extra2=  Objects.requireNonNull(playerIn.getEffect(MobEffects.DAMAGE_BOOST)).getAmplifier();
              }else {
                  extra2=0;
              }
            ...
            for (int i = 0; i < count+extra2; i++)
            {
                SumonSwordEntityEX ss = new SumonSwordEntityEX(LBEntiteRegristrys.ssex, worldIn);

                worldIn.addFreshEntity(ss);

                ss.setSpeed(speed+extra);
                ss.setIsCritical(false);
                ss.setOwner(playerIn);
                ss.setColor(colorCode);
                ss.setRoll(90);
                ss.setDamage((damage+extra)*(1+ (double) extra2 /2));
                // force riding
                ss.startRiding(playerIn, true);
                ss.setPierce((byte) 0);
...
//
    ...
    public static void doSlashex(LivingEntity playerIn, int colorCode, boolean critical, double damage, float speed,int deplays,int count)
    {
        if (playerIn.level().isClientSide()) return;
        ...
        playerIn.getMainHandItem().getCapability(ItemSlashBlade.BLADESTATE).ifPresent((state) -> {
            ...
            for (int i = 0; i < count+extra2; i++)
            {
                ...
                ss.setColor(colorCode);
                ss.setRoll(90);
                ss.setDamage2((damage+extra)*(1+ (double) extra2 /2));
                // force riding
                ss.startRiding(playerIn, true);
                ss.setPierce((byte) 0);
                ss.setDelay(deplays+i);
...
//
    ...
    public static void doSlash(LivingEntity playerIn, int colorCode, boolean critical, double damage, float speed,int deplays,int count,int type)
    {
        if (playerIn.level().isClientSide()) return;
