    ...
    public static void onDoingSlash(SlashBladeEvent.DoSlashEvent event) {
        ...
        if (state.hasSpecialEffect(LBSpecialEffectsRegistry.BlessingSummonSword.getId())) {
            ...
            if (SpecialEffect.isEffective(LBSpecialEffectsRegistry.BlessingSummonSword.get(), level)) {
                    ...
                    BlessingsummonSword.doSlash(player,false, GetplayerattrUtil.getdamage(player),3F);
                    BlessingsummonSword.doSlash(player,false, GetplayerattrUtil.getdamage(player),4F);
                    BlessingsummonSword.doSlash(player,false, GetplayerattrUtil.getdamage(player),5F);
